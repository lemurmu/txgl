...
namespace BizUtils.Sec
{
    ...
    {
        public static bool AllowAccess(string token, string api, DbNetData db)
        {
            bool result = false;
        ...
        public static bool AllowAccess(string token, string api, DbNetDataProxy db)
        {
            DbNetData dbtmp = db.GetDb();
            bool result = AllowAccess(token, api, dbtmp);
            db.PutDb(dbtmp);
            return result;
        }
