            ...
            using (ZipOutputStream zipStream = new ZipOutputStream(returnStream))
            {
                ...
                foreach (var kv in streams)
                {
                    string fileName = kv.Key;
                    {
                        ...
                        float duan = float.Parse(duan_chang.Substring(0, duan_chang.IndexOf('x')));
                        float chang = float.Parse(duan_chang.Substring(duan_chang.IndexOf('x') + 1));
                        using (Stream originStream = File.OpenRead(kv.Value))
                        {
                            using (Bitmap origin = new Bitmap(originStream))
                            {
                                float kuan_inch = 100;
                                float gao_inch = 100;
                                ...
                                origin.SetResolution(xDpi, yDpi);
                                using (MemoryStream streamInput = new MemoryStream())
                                {
                                    origin.Save(streamInput, origin.RawFormat);
                                    streamInput.Flush();
                                    streamInput.Seek(0, SeekOrigin.Begin);
                                    
                                    //BitmapSource myBitmapSource = BitmapFrame.Create(streamInput);
                                    ...
                                    //Console.WriteLine(encoder.CodecInfo);
                                    //encoder.Frames.Add(BitmapFrame.Create(newFormatedBitmapSource));

                                    using (MemoryStream cmykStream = streamInput)
                                    {
                                        //encoder.Save(cmykStream);
                                        cmykStream.Flush();
                                        cmykStream.Seek(0, SeekOrigin.Begin);

                                        int sourceBytes;
                                        do
                                        {
                                            sourceBytes = cmykStream.Read(buffer, 0, buffer.Length);
                                            zipStream.Write(buffer, 0, sourceBytes);
                                        } while (sourceBytes > 0);
                                    }
                                }
